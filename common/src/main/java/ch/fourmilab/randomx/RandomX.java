 ...
 * random generator without the need to reimplement the higher-level calls.
 * <p/>
...
public abstract class RandomX
{
  ...
  /*
   * Emulation of standard java.util.Random methods. Given an implementation of nextByte() in the derived class, these
   * methods provide all the other forms of results. A derived class is, of course, free to reimplement any of these.
   * For example, a generator which produces 31 high-quality pseudorandom bits on each call might reimplement nextBit(),
   * nextShort(), and nextFloat() to avoid the inefficiency of passing individual bytes to the parent class, then
   * reassembling to return to the caller.
   */
